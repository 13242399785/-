<template>
  <div>
    <breadcrumb text="详情列表" />
    <el-card>
      <el-button   type="primary" icon="el-icon-plus" @click="addCate">添加详情</el-button>
      <tree-table
        :data="cateList"
        show-index
        index-text="序号"
    ...
    <!-- 添加详情对话框 -->
    <el-dialog v-dialogDrag title="添加详情" :close-on-click-modal="false" :visible.sync="addDialogVisible" width="50%" @closed="addDialogClose">
    ...
    <!-- 编辑详情对话框 -->
    <el-dialog v-dialogDrag title="编辑详情" :visible.sync="editDialogVisible" :close-on-click-modal="false" width="50%">
...
<script>
// 导入网络请求函数
import { createCate, getCate, updateCate, getCateList, deleteCate } from 'network/admin/detail'
  ...
  methods: {
    ...
    async deleteCateClick(row) {
      try {
        ...
        if (row.children && row.children.length > 0) return this.$message.error('无法删除包含子详情的父级详情')

        const res = await deleteCate(row._id)
        if (!res) return 
        this.$message.success('删除详情成功')
        this.reload()
        this.getCateList()
      } catch (error) {
